...
def Rel_Mutil_men2ent(sequence_output, cls_mask, attention, entity_pos, hts, offset, relation_vector, reduced_dim,
                      Relation_Specific_num, dropout, tag, W):
    """
    ...
    for i in range(len(entity_pos)):
        ...
        hss.append(hs)  # hs,ts - (该bs中所有文档实体对数, Relation_Specific_num+1, d)
        tss.append(ts)
        rss.append(rs)
        Crs.append(Cr_rep_spec)
        ht_atts.append(ht_att)  # 头尾实体对文档关注[ [实体对数, Relation_Specific_num+1, sequence_length] * bs ]
