...
"""
a. 先将graph得到的h t融入hs ts中，之后再进行ET任务
"""
...
"""add_graph"""
# ！
create_graph_ISori = True  # 是否是使用原始图
create_graph_ISori = False  # 是否是使用原始图，否则为add type fa

"""add_sen_graph"""
# ！
create_sentence_graph_ISori = False  # 是否使用带sentence图，默认为false
# create_sentence_graph_ISori = True  # 使用sentence图

#      TODO: 注意Load_feature_file设置false
# ！  易报错   可行：is_entity_pos_fu1 True - entity_pos_isAA True
# ！  易报错   可行：is_entity_pos_fu1 False - entity_pos_isAA False
entity_pos_isAA = True  # 影响entity_emb   True是entity_pos，包含anaphor效果低，    False是entity_pos[:-1]不包含anaphor —— 没有图 or 有图但想用高效果时的emb
is_entity_pos_fu1 = True  # entity_pos if entity_pos[-1] != [] else entity_pos[:-1]

is_entity_pos_fu1 = False
entity_pos_isAA = False  # 影响entity_emb   True是entity_pos，包含anaphor效果低，    False是entity_pos[:-1]不包含anaphor —— 没有图 or 有图但想用高效果时的emb


# ！
cat_graph = False  # True [hs, rs, h_graph] or False [hs, rs] + h_graph
# cat_graph = True
...
fprint("create_sentence_graph_ISori", create_sentence_graph_ISori)
print("*" * 10)

fprint("is_entity_pos_fu1", is_entity_pos_fu1)
print("*" * 10)
print("*" * 10)
