...
def get_pseudo_features(raw_feature: dict, pred_rels: list, entities: list, sent_map: dict, offset: int,
    ...
    for pred_rel in pred_rels:
        ...
        new_entity_pos = []
        new_entity_type = []  # TODO: 增加实体类型加入图中
        ...
        for i, entity in enumerate(entities):
            ...
            for mention in entity:
                if mention["sent_id"] in curr_sents:  # curr_sents 保存证据句
                    ...
                    type_label = docred_ent2id[mention["type"]] - 1  # 去掉NA
                    curr_entity_type.append(type_label)
            ...
            if curr:  # 说明当前实体有提及在伪文档中
                ...
                new_entity_type.append(curr_entity_type)
        ...
        if create_graph_ISori:
            graph = create_graph(new_entity_pos)  # read_docred创建图 - 使用numpy创建邻接矩阵
        else:
            graph = create_graph_add_type(new_entity_pos, new_entity_type)

        if create_sentence_graph_ISori:
            sen_graph = create_sentence_graph(new_entity_pos, new_entity_type, new_sent_pos)
        else:
            sen_graph = None

        if is_entity_pos_fu1:
            new_entity_pos = new_entity_pos if new_entity_pos[-1] != [] else new_entity_pos[:-1]

        pseudo_feature = {'input_ids': new_input_ids,  # 伪文档
                          # 'entity_pos': new_entity_pos if new_entity_pos[-1] != [] else new_entity_pos[:-1],  # 记录实体提及在伪文档中位置
...
def read_docred(file_in,
    ...
    if Load_feature_file and (os.path.exists(feature_file) and teacher_sig_path == "" and single_results == None):
    ...
    for doc_id in tqdm(range(len(data)), desc="Loading examples"):
        ...
        # if sample['title'] == 'Juan Guzm\u00e1n (photographer)':
        #     print()
        ...
        for e in entities:
            ...
            for m in e:
                ...
                type_label = docred_ent2id[label] - 1  # 去掉NA
                entity_pos[-1].append((start, end,))
                entity_type[-1].append(type_label)
        ...
        if create_graph_ISori:
            ...
            graph = create_graph_add_type(entity_pos, entity_type)

        if create_sentence_graph_ISori:
            sen_graph = create_sentence_graph(entity_pos, entity_type, sent_pos)
        else:
            sen_graph = None
