...
class DocREModel(nn.Module):
        ...
        if not self.use_cls_info:
            if self.use_graph:  # 使用图特征
                if cat_graph:
                    ...
                    # hs = torch.tanh(self.head_extractor(torch.cat([hs, rs], dim=-1))) + h_graph  # [1310, 2, 768]   # 60.777827
                    # ts = torch.tanh(self.tail_extractor(torch.cat([ts, rs], dim=-1))) + t_graph
                    hs = torch.tanh(self.head_extractor(torch.cat([hs, rs], dim=-1)) + h_graph)  # [1310, 2, 768]   # 61.098126
                    ts = torch.tanh(self.tail_extractor(torch.cat([ts, rs], dim=-1)) + t_graph)
            else:  # 不使用图特征
    ...
    def forward(self,
        ...
        # rs - (该bs中所有文档实体对数, Relation_Specific_num+1, reduced_dim)，表明该实体对认为重要的上下文嵌入
        # hs, ts - (该bs中所有文档实体对数, Relation_Specific_num+1, reduced_dim), 头实体和尾实体对应的向量集合
        # Crs - [bs * (Relation_Specific_num, reduced_dim)] 关系 所关注的上下文
        # doc_attn 文档中实体对对于整个文档的注意力
        #       (num_ent_pairs_all_batches, Relation_Specific_num+1, doc_len) -> (num_ent_pairs_all_batches, doc_len)
