...
def get_title2pred(pred: list) -> dict:
    ...
    for p in pred:
        ...
        """对于top-k个预测，只保存score最大的预测结果
        2023年论文: Enhancing Document-Level Relation Extraction with Relation- Specific Entity Representation and Evidence Sentence Augmentation
            平均k组伪文档的预测结果 以及 初始文档预测结果 得到最终概率
        """
        if p["title"] in title2pred:
            if curr in title2pred[p["title"]]:  # curr 为 rel_triple
...
def get_title2gt(features: dict) -> dict:
    ...
    for f in features:
        ...
        for idx, p in enumerate(f["hts"]):
            ...
            rs = np.nonzero(label[1:])[0] + 1  # + 1 for no-label  包含了值为非零的元素在原始数组中的索引
...
def select_thresh(cand: list, num_gt: int, correct: int, num_pred: int):
    ...
    Input:
        :cand: list of relation candidates. [(r in title2gt[t], score, t, r[0], r[1], r[2]), ...]
        :num_gt: number of ground-truth relations.
        :correct: number of correct relation predictions selected. 与真实标签对比，预测正确的
        :num_pred: number of relation predictions selected. 预测结果 即在原文档中，也在伪文档中，且都在阈值以上
    Output:
...
def merge_results(pred: list, pred_pseudo: list, features: list, thresh: float = None):
    ...
    Input:
        :pred: topk_results.json
               list of dictionaries, each dictionary entry is a predicted relation triple from the original document.
               Keys: ['title', 'h_idx', 't_idx', 'r', 'evidence', 'score'].
        :pred_pseudo: 伪文档预测结果 list of dictionaries, each dictionary entry is a predicted relation triple from pseudo documents.
                      Keys: ['title', 'h_idx', 't_idx', 'r', 'evidence', 'score'].
        :features: list of features within each document. Identical to the lists obtained from pre-processing.
    ...
    cand = []
    merged_res = []
    correct, num_pred = 0, 0  # correct与真实标签对比，预测正确的     num_pred 预测结果 即在原文档中，也在伪文档中，且都在阈值以上
    ...
    for t in titles:
        ...
        for r in union:  # r表示rel_triple
            if r in rels and r in rels_pseudo:  # 该三元组即出现在原文档中，又出现在伪文档预测中 add those into predictions
                if rels[r] > 0 and rels_pseudo[r] > 0:  # 预测的分数 在 阈值 之上
                    merged_res.append({'title': t, 'h_idx': r[0], 't_idx': r[1], 'r': r[2]})  # 保存确信的结果
                    num_pred += 1
                    correct += r in title2gt[t]
                    continue
                score = rels[r] + rels_pseudo[r]  # 只在原文档 或 只在伪文档预测中存在，则将分数融合，之后放入候选cand中
    ...
    """thresh用于Merge relation predictions，阈值可以最大化test上F1，主要针对于候选cand中那些预测的三元组"""
    if thresh != None:
        sorted_pred = sorted(cand, key=lambda x: x[1], reverse=True)  # 分数降序排序
        last = min(filter(lambda x: x[1] > thresh, sorted_pred))  # 分数大于阈值的最后一个元素
...
def to_official(preds: list, features: list, evi_preds: list = [], scores: list = [], topks: list = []):
    ...
    Input:
        :preds: list of dictionaries, each dictionary entry is a predicted relation triple from the original document.
                Keys: ['title', 'h_idx', 't_idx', 'r', 'evidence', 'score']. (所有文档实体对数, label)
        :features: list of features within each document. Identical to the lists obtained from pre-processing.
        :evi_preds: list of the evidence prediction corresponding to each relation triple prediction. (396790, 25)
    ...
    for i in tqdm(range(preds.shape[0]), desc="preds"):  # for each entity pair
        if scores != []:
            score = extract_relative_score(scores[i], topks[i])  # 计算top-k的score相对分数
            pred = topks[i]
        else:
            pred = preds[i]
...
def official_evaluate(tmp, path, train_file="train_annotated.json", dev_file="dev.json"):
    ...
    for x in truth:
        ...
        for label in x['labels']:
            ...
            h_idx = label['h']
            t_idx = label['t']
            std[(title, r, h_idx, t_idx)] = set(label['evidence'])  # 保存真实的三元组
    ...
    for x in submission_answer:
        ...
        if (title, r, h_idx, t_idx) in std:  # 真实三元组
            correct_re += 1
