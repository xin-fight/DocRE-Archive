...
"""改进版本  在哪个阶段使用topk计算的结果，其他阶段使用 数据集标记的type"""
TSER_topk_tag = ["test", "dev", "train"]  # 这是最原始的版本，无论什么都会直接使用topk进行预测
TSER_topk_tag = ["train"]
# TSER_topk_tag = []

"""不同向量构建"""
change_rsh = False  # False [hs, rsh]       True [rsh, hs] - * 之前使用的是False
# change_rsh = True  # False [hs, rsh]      True [rsh, hs]   我认为True应该是合理的
ET_hs_old = True  # False 融合了上下文          True [old_hs, old_ts] - 实体类型分类时实体嵌入的构成    * 之前使用的是True

add_old = True  # 是否在构造新向量时，使用之前的向量

"""SAIS下：TSER & Logsumexp"""
Hs_Ts_list = "TSER_Mean_Ground"  # cat(mean(hs), hs_new)  mean+正确type           NB
# Hs_Ts_list = "Type_Ground_True"  # 使用正确的type
# Hs_Ts_list = "Mean"  # mean

"""fs
* bernoulli_list: ['train', 'test']
TSER_topk_tag []   Hs_Ts_list: TSER_Mean_Ground  61.441952
TSER_topk_tag []   Hs_Ts_list: Type_Ground_True  61.156109
TSER_topk_tag []   Hs_Ts_list: Mean  61.341937
TSER_topk_tag ["train"]   Hs_Ts_list: TSER_Mean_Ground  61.502920 ！
    bernoulli_list ["train"] 61.502920
TSER_topk_tag ["train"]   Hs_Ts_list: Type_Ground_True  61.011110
TSER_topk_tag ["train"]   Hs_Ts_list: Mean  61.341937

TSER_topk_tag = ["test", "dev", "train"]   Hs_Ts_list: TSER_Mean_Ground  61.504945
"""
...
path = ("add_old{}change_rsh{}Topk{}Hs_Ts_list{}REDocred_TASK_ET{}{}_{}_s_attn_lambda{}_three{}_cls{}_relation{}_extractor{}_seed{}_Relation_Seploss{}_Crht_loss{}_Emb_loss{}"
        "_Relation_loss{}").format(add_old, change_rsh, str(len(TSER_topk_tag)), Hs_Ts_list, TASK_ET, loss_weight_ET,
                                   evi_lambda, s_attn_lambda, three_atten, use_cls_info, relation_dim, extractor_dim, seed,
                                   Relation_Seploss, Crht_loss, Emb_loss, Relation_loss
                                   )
save_path = "./Save/" + path
...
print("##" * 10)
fprint("Hs_Ts_list", Hs_Ts_list)
print("_" * 20)
fprint("TSER_topk_tag", TSER_topk_tag)
fprint("bernoulli_list", bernoulli_list)
print("_" * 20)
fprint("change_rsh", change_rsh)
fprint("ET_hs_old", ET_hs_old)
fprint("add_old", add_old)
