...
def Rel_Mutil_men2ent(sequence_output, cls_mask, attention, entity_pos, hts, offset, relation_vector, reduced_dim,
    ...
    hss, tss, rss, rss_old, Crs = [], [], [], [], []
    ht_atts, ht_atts_old, bs_ht_num = [], [], [0]
    ...
    for i in range(len(entity_pos)):
        ...
        # 包含该文档中的所有实体的表示
        entity_embs = torch.stack(entity_embs, dim=0)  # [n_e, Relation_Specific_num+1, d]
        entity_atts = torch.stack(entity_atts, dim=0)  # [n_e, Relation_Specific_num+1, h, seq_len]

        # hs,ts - (实体对数, Relation_Specific_num+1, d)  ht_i：该bs包含了实体和除了本身之前的 所有实体对
        ht_i = torch.LongTensor(hts[i]).to(sequence_output.device)
        hs = torch.index_select(entity_embs, 0, ht_i[:, 0])
        ts = torch.index_select(entity_embs, 0, ht_i[:, 1])

        # h_att, t_att - [实体对数, Relation_Specific_num+1, num_heads, sequence_length]
        h_att = torch.index_select(entity_atts, 0, ht_i[:, 0])
        t_att = torch.index_select(entity_atts, 0, ht_i[:, 1])

        # !!!!!!!!!!!!!!!!!!!!!!!! 这里需要变成 type_h * type_t
        ...
        """[实体对数, Relation_Specific_num+1, Relation_Specific_num+1, num_heads, sequence_length]"""
        # [实体对数, R+1, R+1, h, sequence_length] -> [实体对数, R+1, R+1, sequence_length]
        ht_att = torch.einsum('nRhl,nrhl->nRrhl', h_att, t_att).mean(3)  # average over all heads
        ht_att = ht_att / (ht_att.sum(3, keepdim=True) + 1e-30)  # [实体对数, R+1, R+1, seq]
        """localized context embedding
        [实体对数, Relation_Specific_num+1, Relation_Specific_num+1, reduced_dim]"""
        rs = contract("ld,nRrl->nRrd", sequence_output[i], ht_att)

        """Old_rs_cal"""
        if add_old:
            ht_att_old = (h_att * t_att).mean(2)  # average over all heads
            # [实体对数, Relation_Specific_num+1, sequence_length]  归一化，使它们的总和等于 1
            ht_att_old = ht_att_old / (ht_att_old.sum(2, keepdim=True) + 1e-30)
            # obtain local context embeddings.  [实体对数, Relation_Specific_num+1, reduced_dim]
            rs_old = contract("ld,nal->nad", sequence_output[i], ht_att_old)
        else:
            rs_old = []
        ...
        法2）
        """
        rs_Csor = rs[:, 1:, 1:, :]  # [实体对数, Relation_Specific_num, Relation_Specific_num, reduced_dim]

        """最新版本 Crs 所关注的上下文 --- 用于Crht_loss"""
        Cr_rep_spec = rs_Csor  # [实体对数, R, R, reduced_dim]

        """原版计算 Crs 所关注的上下文 --- 用于Crht_loss"""
        ...
        #
        # # rs_Csor [实体对数, Relation_Specific_num, Relation_Specific_num, reduced_dim]
        # # relation_vector [Relation_Specific_num, reduced_dim]
        ...
        hss.append(hs)  # [实体对数, R+1, reduced_dim]  pooling+R
        tss.append(ts)
        rss.append(rs)  # localized context embedding  [实体对数, R+1, R+1, reduced_dim] mean+R
        rss_old.append(rs_old)  # localized context embedding  [实体对数, Relation_Specific_num+1, reduced_dim] mean+R

        Crs.append(Cr_rep_spec)  # Crs 所关注的上下文 [ [实体对数, R, R, reduced_dim] * bs ] R
        ht_atts.append(ht_att)  # 头尾实体对文档关注 [ [实体对数, R+1, R+1, seq] * bs ] mean+R
        if add_old:
            ht_atts_old.append(ht_att_old)  # 头尾实体对文档关注 [ [实体对数, R+1, R+1, seq] * bs ] mean+R
        else:
            ht_atts_old.append([])

        bs_ht_num.append(bs_ht_num[-1] + len(hs))

    return hss, tss, rss, rss_old, Crs, ht_atts, ht_atts_old, bs_ht_num
