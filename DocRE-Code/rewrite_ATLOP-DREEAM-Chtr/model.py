...
from losses import ATLoss, MLLTRSCLloss, Relation_Specificloss
from constant import *
from Model_assistant import *
...
class DocREModel(nn.Module):
    def __init__(self, config, model, tokenizer,
        ...
        """清空中间结果，分析证据句"""
        with open('env_sent.pkl', 'wb') as f:  # 使用追加模式打开文件
            pickle.dump([], f)
    ...
    def forward_rel(self, hs, ts, rs):
        ...
        b1 = hs.view(-1, self.Relation_Specific_num + 1, self.extractor_dim // self.block_size, self.block_size)
        b2 = ts.view(-1, self.Relation_Specific_num + 1, self.extractor_dim // self.block_size, self.block_size)
        bl = (b1.unsqueeze(4) * b2.unsqueeze(3)).view(-1, self.Relation_Specific_num + 1, self.extractor_dim * self.block_size)
    ...
    def forward(self,
        ...
        # doc_attn 文档中实体对对于整个文档的注意力 (num_ent_pairs_all_batches, doc_len)
